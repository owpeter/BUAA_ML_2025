...
以二分类为例，形式化的，我们记第$i$类样本均值为$\tilde{m_i}$，第$i$样本类内离散度为
$$
\begin{aligned}
...
我们希望投影后在一维$\mathbf{Y}$空间中的样本尽量分开，即两类均值之差$\tilde{m_1} - \tilde{m_2}$越大越好。综合来看，我们就能够得到Fisher准则函数：
...
SVM 最早由 C. Cortes 和 V. Vapnik 于1995年提出，其理论基础是统计学习理论 (Statistical Learning Theory, SLT)。简单来说，SVM 的核心思想是在特征空间中找到一个能够将不同类别样本最好地分离开的超平面（在二维空间中可以理解为一条直线）。所谓“最好”，指的是这个超平面不仅能正确地将两类样本分开，而且使得**离它最近的样本**点（即**支持向量**）到该超平面的间隔最大化。这个间隔被称为“边距 (Margin)”。
...
解决这个优化问题通常采用拉格朗日乘子法，将其转化为对偶问题。在求解过程中，KKT (Karush-Kuhn-Tucker) 条件起着关键作用，它揭示了只有那些位于间隔边界上的样本点（即支持向量）才对最终的分类超平面有贡献，而其他样本点则无关紧要。
