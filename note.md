...
假设现在有一组样本向量$x_1, x_2, ..., x_N$分别来自两个类别$\chi_1, \chi_2$，如果存在一个权重向量$w$，使得$\forall x\in \chi_1,w^tx\gt 0$，$\forall x\in \chi_2,w^tx\lt 0$，那么称这组样本是线性可分的。

现在我们希望寻找一个权重向量，使得$w^tx\gt 0, \forall x\in \chi_i$，那么我们可以构造一个准则函数，它的大小取决于被错分类的样本。

$$
\begin{aligned}
    J_P(w) = \sum_{-x\in \chi_k}(w^Tx)
\end{aligned}
$$

其中$chi_k$表示被错分类的样本集合。当$x$被错分类是，$w^Tx\lt 0$，那么$J_P(w) \gt 0$，因此我们的目标是找到一个解向量$a*$，使得$J_P(a*)=0$，这意味着错分类集合为空

我们可以使用梯度下降法求解解向量：

将$J_P(w)$对$w$求偏导，得到$\nabla J_P(w) = \sum_{x\in \chi_k}(-x)$，那么每轮迭代权重更新的表达式为
...
其中$\eta$是一个超参数，称为**学习率**

- 感知机准则的结果不唯一，且在样本**线性不可分**的时候无法收敛

3. **最小二乘准则 (Least Squares Criterion)**

旨在最小化预测值与目标值之间误差的平方和。对于分类问题，可以为不同类别的样本设置不同的目标值（例如，属于类别 $w_1$ 的样本目标值为 $b_1 > 0$，属于类别 $w_2$ 的样本目标值为 $b_2 < 0$，通常是将样本规范化后，目标值设为正数）。设增广样本矩阵为 $\mathbf{Y}$，目标值向量为 $\mathbf{b}$，则目标是找到权向量 $\mathbf{a}$ 使得 $J_S(a)=\|\mathbf{Y}\mathbf{a} - \mathbf{b}\|^2$ 最小。其解析解（伪逆解）为 $\mathbf{a}^* = (\mathbf{Y}^T\mathbf{Y})^{-1}\mathbf{Y}^T\mathbf{b}$。也可以使用梯度下降法求解。最小二乘法对异常值（Outlier）非常敏感。
...
为了找到这个最优超平面，我们需要找到一个最大几何间隔：

$$
\begin{aligned}
    \frac{t_n(\omega^T x_n+b)}{||\omega||}
\end{aligned}
$$

而最大化几何间隔等价于最小化 $||\omega||$，或者更方便的，最小化 $\frac{1}{2} ||\omega||^2$，约束条件是对于所有样本 $n$，$t_n(w^T x_n + b) \ge 1$（其中 $t_n$ 是样本的类别标签，+1或-1）。

解决这个优化问题通常采用拉格朗日乘子法，将其转化为对偶问题。在求解过程中，KKT (Karush-Kuhn-Tucker) 条件起着关键作用，它揭示了只有那些位于间隔边界上的样本点（即那些满足$t_i(w^Tx_i+b)=1$的向量，我们称其为**支持向量**）才对最终的分类超平面有贡献，而其他样本点则无关紧要。

**2. 处理噪声与离群点：软间隔SVM的引入**

在现实世界的数据中，样本往往**不是完全线性可分**的，或者存在一些噪声和离群点。硬间隔SVM在这种情况下可能会表现不佳，甚至找不到合适的分类面。为了解决这个问题，软间隔SVM被提了出来。

软间隔SVM允许一些样本点被错误分类或者进入间隔区域，通过引入松弛变量 $\xi_n \ge 0$ 来衡量这种“容错程度”。优化目标函数也相应地修改为：最小化 $\frac{1}{2} ||w||^2 + C \sum_{n=1}^{N} \xi_n$。其中，$C$ 是一个惩罚系数，用于权衡间隔大小和错分样本的数量。$C$ 越大，对错分的惩罚越重，模型越接近硬间隔SVM；$C$ 越小，对错分的容忍度越高。
...
然而，直接进行高维映射和计算会带来巨大的计算量（维度灾难）。核技巧的巧妙之处在于，它允许我们直接在原始的低维空间中计算一个值，这个值等同于数据点在经过某个非线性映射 $\phi$ 后在高维特征空间中的点积，即
...
选择合适的核函数对于非线性SVM的性能至关重要，通常需要根据具体问题和数据特性进行选择。
...
因此我们的目标就是最大化$u_1^TSu_1$，同事有等式约束$u_1^Tu_1=1$。利用拉格朗日乘子法并对$u_1$求导置0，得到$Su_1 = \lambda_1u1$，进一步得到$u_1^TSu_1=\lambda_1$，其中$\lambda_1$是矩阵$S$的特征值。根据要求，$\lambda_1$是所有特征值中最大的那个，那么$u_1$就是矩阵$S$最大特征值对应的特征向量，称为**第一主成分**，它指出了包含信息最多的一个方向
